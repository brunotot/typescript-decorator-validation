...
export default function useValidation<T>({
  ...
  const [value, setValue] = useState<T>(defaultValue ?? processor.buildEmptyInstance());
  const [detailedErrors, setDetailedErrors] = useState<DetailedErrors<T>>({} as DetailedErrors<T>);
