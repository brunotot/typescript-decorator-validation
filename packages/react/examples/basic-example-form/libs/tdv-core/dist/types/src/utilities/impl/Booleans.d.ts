 ...
 */
export type isObject<T> = NonNullable<T> extends isFunction<T> ? false : NonNullable<T> extends isArray<T> ? false : NonNullable<T> extends isPrimitive<T> ? false : true;
/**
 * Checks if `T` is a function type.
 *
 * @typeParam T - The type to check.
 */
export type isFunction<T> = NonNullable<T> extends Types.FunctionType ? Types.UnwrapPromise<ReturnType<NonNullable<T>>> extends API.Validation.ValidationResult ? true : false : false;
/**
